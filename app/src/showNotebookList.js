define(['jquery', 'data'], function($, data) {
  ...
  var showNotebooks = function() {
    if (notebooks.length === 0) {
      $("#wn-notebooks").append("<div>您暂时未添加笔记本<div>");
    } else {
      $("#wn-notebooks").append("<ul></ul>");
      for (var i = 0; i < notebooks.length; i++) {
        $("#wn-notebooks>ul").append("<li>" + notebooks[i].title + " " + notebooks[i].createDate + " " + notebooks[i].modifyDate + "</li>");
      }
    }
  };
  ...
  var showNotes = function(id) {
    if (notebooks[id].notes.length === 0) {
      $("#wn-notes").append("<div>您暂时未在本笔记本添加笔记<div>");
    } else {
      $("#wn-notes").append("<ul></ul>");
      for (var i = 0; i < notebooks[id].notes.length; i++) {
        $("#wn-notes>ul").append("<li>" + notebooks[id].notes[i].title + " " + notebooks[id].notes[i].createDate + " " + notebooks[id].notes[i].modifyDate + " " + notebooks[id].notes[i].tag + " " + notebooks[id].notes[i].content.substring(0, 6) + "</li>");
      }
    }
  };
});